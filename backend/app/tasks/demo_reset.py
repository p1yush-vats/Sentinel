...
async def revert_demo_admin_changes():
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            logger.warning(f"Demo admin setup attempt {attempt}/{max_retries} failed: {e}. Retrying in {wait}s...")
            if attempt == max_retries:
                logger.error("All retries exhausted. Demo admin not set up this cycle.")
                return
