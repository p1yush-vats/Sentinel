...
class SessionManager:
    ...
    async def start_session(self, force_end_existing: bool = False) -> Dict:
        ...
        Creates session in both TimeEngine and Backend
        """
        # Start local session
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    print(f"❌ Backend error ({response.status_code}): {error_text}")
                    
                    # Backend failed, queue for later
    ...
    async def end_session(self) -> Dict:
        ...
        Ends session in both TimeEngine and Backend
        """
        # End local session
                ...
                async with httpx.AsyncClient(timeout=10.0) as client:
                    response = await client.post(
                        ...
                        json={
                            ...
                            "session_quality_score": 100.0  # TODO: Calculate quality
                        }
                    )
    ...
    async def create_work_log(
        ...
        Args:
            log_type: work, break, lunch
            start_time: Log start time
            end_time: Log end time
            duration_minutes: Duration in minutes
            break_token_used: Whether break token was used
    ...
    def update(self) -> Dict:
        ...
        if self.last_sync_time is None or \
           (datetime.now() - self.last_sync_time).total_seconds() > self.sync_interval_seconds:
            # Schedule sync (don't block UI)
            # In production, run this in background thread
            pass
