...
const EVENT_CONFIG = {
  ...
  task_assigned:          { dot: 'bg-violet-400',  label: '📋 Task Assigned',             desc: (e) => `"${e.metadata?.title || 'New task'}" · ${e.metadata?.priority || 'medium'} priority` },
  task_completed:         { dot: 'bg-emerald-400', label: '✅ Task Completed',             desc: (e) => `"${e.metadata?.title || 'Task'}" marked done` },
}
...
export default function LiveFeed() {
  ...
  // Handle real-time events from backend WebSocket
  useEffect(() => {
    if (!lastMessage) return

    // Backend broadcasts: abnormality_detected, session_update, task_assigned, task_completed
    if (lastMessage.type === 'abnormality_detected') {
      const d = lastMessage.data
      const detectionType = d.detections ? Object.keys(d.detections)[0] : 'unknown'
      setEvents(prev => [{
        id:         d.id || `ws-${Date.now()}`,
        event_type: 'abnormality_detected',
        created_at: d.last_updated_at || new Date().toISOString(),
        metadata:   { type: detectionType, confidence: d.confidence_score, severity: d.overall_severity }
      }, ...prev].slice(0, 20))
    }

    if (lastMessage.type === 'session_update') {
      const d = lastMessage.data
      const isEnd = d.status === 'completed'
      setEvents(prev => [{
        id:         d.id || `ws-${Date.now()}`,
        event_type: isEnd ? 'session_ended' : 'session_started',
        created_at: d.start_time || new Date().toISOString(),
        metadata:   { work_minutes: d.total_work_minutes, break_minutes: d.total_break_minutes }
      }, ...prev].slice(0, 20))
    }
    ...
    if (lastMessage.type === 'task_assigned') {
      ...
      setEvents(prev => [{
        ...
        metadata:   { title: d.title, priority: d.priority }
      }, ...prev].slice(0, 20))
    }
    ...
    if (lastMessage.type === 'task_completed') {
      ...
      setEvents(prev => [{
        ...
        created_at: d.completed_at || new Date().toISOString(),
      ...
      <div className="overflow-y-auto flex-1 -mx-1 px-1">
        {loading ? (
          ...
          events.map((e, i) => {
            ...
            return (
              <div key={e.id || i} className="flex gap-3 py-2.5 border-b border-sentinel-border/30 last:border-0">
